...
type Accesspoint struct {
	...
	// baseCtx is the session-scoped context. ctx is derived from it and canceled
	// on Close, so both session shutdown and AP shutdown bound the reconnect
	// backoff and in-flight I/O. Reads/writes on the underlying conn are still
	// interrupted via conn.Close as before.
...
func NewAccesspoint(log librespot.Logger, addr librespot.GetAddressFunc, deviceId string, baseCtx context.Context) *Accesspoint {
	...
	return &Accesspoint{log: log, baseCtx: baseCtx, ctx: ctx, cancel: cancel, addr: addr, deviceId: deviceId, done: make(chan struct{}), recvChans: make(map[PacketType][]chan Packet)}
}
