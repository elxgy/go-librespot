...
// TestPauseDuringCrossfade: pausing mid-fade means the output simply stops
// reading; state must survive untouched and the fade must resume exactly
// where it stopped when reads resume. This was a blind spot upstream.
func TestPauseDuringCrossfade(t *testing.T) {
	const fade = 400
	a := constSource(2000, 0.5, 0)
	b := constSource(2000, 0.25, 0)
	...
	for {
		_, err := s.Read(buf)
		if err != nil {
			t.Fatalf("unexpected error before fade: %v", err)
		}
		if s.fading {
			break
		}
	}
	fadePosAtPause := s.fadePos

	// "pause": reads stop for a while; the output driver stops reading and
	// nothing calls into the source. PositionMs may still be polled.
	for i := 0; i < 10; i++ {
		_ = s.PositionMs()
		if !s.fading {
			t.Fatal("position polling must not abort an in-flight fade")
		}
	}

	// "resume": reads continue and the fade completes from where it stopped.
	fadePosAtResume := s.fadePos
	if fadePosAtResume < fadePosAtPause {
		t.Fatalf("fade went backwards while paused: %d < %d", fadePosAtResume, fadePosAtPause)
	}
	total := 0
	for {
		n, err := s.Read(buf)
		...
		total += n
		if total > 4*len(a.samples)+4*len(b.samples) {
			t.Fatal("read loop did not terminate after fade")
		}
	}
	if s.fading {
